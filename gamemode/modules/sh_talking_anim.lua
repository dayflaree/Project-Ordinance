...
MODULE.author = "Riggs & Dayflare"

if ( SERVER ) then
    util.AddNetworkString("ax.talking_anim.chat_open")
else
    hook.Add("StartChat", "ax.talking_anim.chat_open", function()
        if ( !IsValid(LocalPlayer()) ) then return end

        net.Start("ax.talking_anim.chat_open")
        net.SendToServer()
    end)

    hook.Add("ChatboxOpened", "ax.talking_anim.chat_open", function()
        if ( !IsValid(LocalPlayer()) ) then return end
...
if ( SERVER ) then
    net.Receive("ax.talking_anim.chat_open", function(_, client)
...
function MODULE:PlayerStartVoice(client)
    if ( !ax.util:IsValidPlayer(client) or !client:Alive() ) then return end
    if ( !client:RateLimit("talking.anim.voice", 0.3) ) then return end
